...
public class ResourceTest {
    ...
    public void compileResource() {
        assertThat(loadResourceNamed("compile-resource.txt").trim()).isEqualTo("compile");
    }

    @Test
    public void testResource() {
        assertThat(loadResourceNamed("test-resource.txt").trim()).isEqualTo("test");
    }
    ...
    private String loadResourceNamed(String resource) {
        try (BufferedReader stream = new BufferedReader(new InputStreamReader(Objects.requireNonNull(Thread.currentThread().getContextClassLoader().getResourceAsStream(resource)), UTF_8))) {
            ...
            throw new RuntimeException(String.format("Failed to load resource \"%s\"", resource), e);
        }
    }
}
